<?php
  ...
  if(isset($_POST["user_data_submit"])){
	...
	//kuupäeva valiidsus ehk reaalsuse kontrolli
	if(empty($birth_day_error) and empty($birth_month_error) and empty($birth_year_error)) {
	 ...
	 if(empty($name_error) and empty($surname_error) and empty($birth_month_error) and empty($birth_year_error) and empty($birth_date_error) and empty($gender_error) and empty($email_error) and empty($password_error) and empty($confirm_password_error)) {
		$notice = sign_up($name,$surname, $gender, $birth_date, $email, $_POST["password_input"]);
		if($notice == 1) {
			$notice = "Uus kasutaja on edukalt loodud!";
		}
		elseif($notice == 2) {
			$notice = "Sellise kasutajatunnusega kasutaja on juba olemas!";
		}
		else {
...
<html lang="et">
  ...
  <body>
	...
	<form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <input type="radio" name="gender_input" value="3" <?php if($gender == "3"){		echo " checked";} ?>><label>Naine</label>
	  <input type="radio" name="gender_input" value="2" <?php if($gender == "2"){		echo " checked";} ?>><label>Mees</label>
	  <input type="radio" name="gender_input" value="1" <?php if($gender == "1"){		echo " checked";} ?>><label>Muu</label><br>
